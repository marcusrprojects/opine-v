...
const CategoryDetail = () => {
  ...
  const { user } = useAuth();
  const { isFollowing, userData } = useUserData();
  const { likedCategories } = userData || {};

  const [category, setCategory] = useState(null);
  const [items, setItems] = useState([]);
  // orderedFields now are objects with { id, name, active }
  ...
  // filterFieldsSelected is an array of objects: { id, name }
  ...
  // For filtering, use only active fields.
  const activeFilterFields = filterFields;
  ...
  // Only include active fields from orderedFields.
  const activeOrderedFields = orderedFields.filter((field) => field.active);
  // Compute final ordered fields (based on active fields and selected filters).
  ...
  // Updated deletion: call secure backend endpoint.
  ...
  // Updated like toggle: call secure endpoint.
  const handleToggleLike = async () => {
    try {
      const response = await fetch(`/api/toggleLikeCategory`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ categoryId }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Toggle like failed");
    } catch (error) {
